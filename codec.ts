...
const KEYABLE_PAIR_DELIM = ' ' // space
...
const zipPairs = (pairs: Pair[], range: number[]) =>
...
function getBpeRanks(bpeContent: string) {
	const pairs = getBpePairs(bpeContent)
	return zipPairs(pairs, range(0, pairs.length))
}

export function getBpeRankFrom(bpeContent: string): GetBpeRank {
	const ranks = Object.assign(Object.create(null), getBpeRanks(bpeContent))
...
globalThis.Deno?.test('getBpePairs', async () => {
